...
<body>
    ...
    <script>
        var headOperationCode = 0;//0代表添加、1代表修改、
    </script>

    <script src="<%=request.getContextPath() %>/statics/js/headOperation.js"></script>

    <%--导航--%>
    <ul class="nav nav-tabs">
        <li><a href="headOperation_returnIndex">主页</a></li>
        <li><a href="modelOperation_modelAdminPage.action">模块管理</a></li>
        <li class="active"><a href="#">负责人管理</a></li>
    </ul>

    <%--按钮组--%>
    <div class="btn-group">
        <button type="button" class="btn btn-primary btn-lg" id="addHeadButton">新增</button>
    </div>
    ...
    <script>
        ...
        function AddFunctionAlty(value, row, index) {
            return '<button id="updateItemButton" type="button" class="btn btn-default" onclick="update(' + row.headid +')">修改</button>' +
                '<button id="TableView" type="button" class="btn btn-default" onclick="delPost(' + row.headid +')" style="margin-left: 10px;">删除</button>';
    ...
    <script>
        $('#add-submit').click(
            function () {
                //判断是否选择正确、
                if($('#headInput').val().trim() == ""){//没有选择、
                    alert("您还没有填写完整、请检查、")
                    return;
                }
                switch (headOperationCode) {
                    case 0:
                        var row = {
                            headid :     "0",
                            headname :     $('#headInput').val()
                        };
                        addPost(row);
                        break;
                    case 1:
                        var row = {
                            headid :     $('#headIDInput').val(),
                            headname :     $('#headInput').val()
                        };
                        updatePost(row);
                        break;
